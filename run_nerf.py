...
wandb.init(project="NeRF", notes="lossChange", tags=["custom_loss","group_normalization"])
wandb.config = {
        'Iterations': 200000,
        'DATASET': "COCO",
        'LossFunction': "MS-SSIM + L1",
        'weight': 1,
        'gaussian_filter size': 3,
        'N_rand': 32*32*4,
        'batchify_ray': 1024*32
    }
...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    sh = rays_d.shape # [378, 504, 3]
    if ndc: # ray to Normalized Device Coordinates (NDC)
    ...
    rays = torch.cat([rays_o, rays_d, near, far], -1)   # [378*504,8]
    if use_viewdirs:
        rays = torch.cat([rays, viewdirs], -1)

    # Render and reshape
    all_ret = batchify_rays(rays, chunk, **kwargs)
    for k in all_ret:   # iterate for key in dictionary
        k_sh = list(sh[:-1]) + list(all_ret[k].shape[1:]) # exclude last dim in sh, load 2nd and so on in all dim
        all_ret[k] = torch.reshape(all_ret[k], k_sh)

    # k_extract = ['rgb_map', 'disp_map', 'acc_map']  #extract key
        k_extract = ['rgb_map', 'disp_map', 'acc_map', 'pt_map', 'disp_mask']  #extract key
    ret_list = [all_ret[k] for k in k_extract]  # save in the list for the corresponding Result
...
def config_parser():
    ...
    parser.add_argument("--get_rect", action='store_true', 
                        help='get rectangular sample for MS-SSIM')
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            img_i1, img_i2 = random.sample(list(i_train), 2)

            target = images[img_i1]
            target = torch.Tensor(target).to(device)
            pose = poses[img_i1, :3,:4]
            ...
            if N_rand is not None:
                ...
                # if args.get_rect:
                #     sample_img = images[img_i1]
                #     sample_img = torch.Tensor(sample_img).to(device)

                #     sample_mask_rect = mask_rect[f'{img_i1:03d}_{img_i2:03d}']

                #     select_coords, _ = get_sample_rect(args.Patch_H, args.Patch_W, sample_mask_rect, sampled_coords, probs)
                #     select_coords = select_coords.long()
                #     rays_o = rays_o[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3)
                #     rays_d = rays_d[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3)
                #     batch_rays = torch.stack([rays_o, rays_d], 0) # (2, N_rand, 3)

                #     target_s = sample_img[select_coords[:, 0], select_coords[:, 1]]  # (N_rand, 3) 
                ...
                print(f'\nN_rand {args.N_rand}, {N_rand}, {rays_o.shape}\n')
                # print(f'batch_rays in train {batch_rays}\n')
                # print(f'rays_o, rays_d in train {rays_o}\n{rays_d}\n')

        #####  Core optimization loop  #####
        rgb, disp, acc, pt, disp_mask, unprojected_pt, extras = render(H, W, K, chunk=args.chunk, rays=batch_rays,
                                                                        verbose=i < 10, retraw=True,
                                                                        **render_kwargs_train)
        # print(f'rgb all 1? {torch.all(rgb.eq(1))}')
        
        optimizer.zero_grad()
        # img_loss = img2mse(rgb, target_s)

        # # Calculate L1 loss
        # l1_loss = torch.abs(rgb - target_s).mean()
        # # Note that MS-SSIM might require normalized values, e.g., in the range of [0, 1]
        ...
        rgb_for_ssim = torch.reshape(rgb, [args.Patch_H, args.Patch_W, 3])  # [ n, n, 3]
        target_for_ssim = torch.reshape(target_s, [args.Patch_H, args.Patch_W, 3])  # [n, n, 3]
        rgb_for_ssim = rgb_for_ssim.view(1,3,args.Patch_H,args.Patch_W)
        target_for_ssim = target_for_ssim.view(1,3,args.Patch_H,args.Patch_W)
        
        ssim_loss = torch.mean(ssim_fun(target_for_ssim.unsqueeze(0), rgb_for_ssim.unsqueeze(0)))
        
        loss = img_loss + (1 - ssim_loss)
       
        if 'rgb0' in extras:
            # # img_loss0 = img2mse(extras['rgb0'], target_s)
            # # loss = loss + loss0 img_loss0
            # l1_loss0 = torch.abs(extras['rgb0'] - target_s).mean()
       
            img_loss0 = img2mse(extras['rgb0'], target_s)
            psnr0 = mse2psnr(img_loss0)
            
            rgb0_for_ssim = torch.reshape(extras['rgb0'], [args.Patch_H, args.Patch_W, 3])  # [ n, n, 3]
            target0_for_ssim = torch.reshape(target_s, [args.Patch_H, args.Patch_W, 3])  # [n, n, 3]
            rgb0_for_ssim = rgb0_for_ssim.view(1,args.Patch_H,args.Patch_W)
            target0_for_ssim = target0_for_ssim.view(1,3,args.Patch_H,args.Patch_W)


            ssim_loss0 = torch.mean(ssim_fun(target0_for_ssim.unsqueeze(0), rgb0_for_ssim.unsqueeze(0)))

            loss = loss + img_loss0 + (1 - ssim_loss0)
        # print(f'img loss, ssim, ssim0: {img_loss}, {ssim_loss}, {ssim_loss0}')
